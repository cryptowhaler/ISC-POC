"""
Various tool function for TradingBot scripts
"""
import gzip
import logging
import re
...
def render_template(templatefile: str, arguments: dict = {}) -> str:
    ...
    env = Environment(loader=PackageLoader('tradingbot', 'templates'), autoescape=select_autoescape(['html', 'xml']) ) #creates env
    template = env.get_template(templatefile)      #opens the template file (name) present in the env
    return template.render(**arguments)             #assigns values to the variables in the template file (**arguments as passing a dict)
