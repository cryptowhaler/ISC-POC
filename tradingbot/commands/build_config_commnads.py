...
from tradingbot.constants import UNLIMITED_STAKE_AMOUNT
from tradingbot.exchange import available_exchanges, MAP_EXCHANGE_CHILDCLASS
from tradingbot.misc import render_template
...
def ask_user_overwrite(config_path: Path) -> bool:
    ...
    answers = prompt(questions)  #prompt function used to display msg on console and stores the response as a dictionary with key = name = "overwrite"
...
def deploy_new_config(config_path: Path, selections: Dict[str, Any]) -> None:
    ...
    try:
        ...
        selections['exchange'] = render_template(templatefile=f"subtemplates/exchange_generic.j2", arguments=selections)
...
def start_new_config(args: Dict[str, Any]) -> None:
    ...
    print("CHECKIING  GGG")
    print(args['config'])
    config_path = Path( args['config'][0] )  #returns Path object type
    if config_path.exists():
        overwrite = ask_user_overwrite(config_path)
        if overwrite:
            config_path.unlink()  #deletes the file / dir at the path
        else:
